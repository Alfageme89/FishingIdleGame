...
class GameRepository(context: Context) {
    ...
    private val prefs: SharedPreferences =
        context.getSharedPreferences("fishing_prefs", Context.MODE_PRIVATE)
    ...
    suspend fun saveProgress(state: GameState) = withContext(Dispatchers.IO) {
        prefs.edit().apply {
            ...
            state.speciesCounts.forEach { (name, count) ->
                putInt(k("count_$name"), count)
            }

            state.maxWeights.forEach { (name, weight) ->
                putFloat(k("weight_$name"), weight)
            }

            state.upgLevels.forEach { (key, level) ->
                putInt(k("upg_$key"), level)
            }
    ...
    suspend fun loadProgress(): GameState = withContext(Dispatchers.IO) {
        ...
        val caughtSpecies =
            prefs.getStringSet(k("caughtSpecies"), emptySet()) ?: emptySet()
